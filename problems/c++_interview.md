...
### 2.9、C++中四种类型转换是：static_cast, dynamic_cast, const_cast, reinterpret_cast
...
1）比如非const转const；
2）void*转指针等；
3）static_cast能用于多态向上转化；
4）如果向下转能成功但是不安全，结果未知；

3. dynamic_cast

用于动态类型转换。
1）只能用于含有虚函数的类，用于类层次间的向上和向下转化。
2）只能转指针或引用。
3）向下转化时，如果是非法的对于指针返回NULL，对于引用抛异常。要深入了解内部转换的原理。
...
sizeof(a)时候求得是上述表格数值，（字节数）1字节=8bits，避免使用long类型，造成跨平台的错误。
...
[参考资料](https://blog.csdn.net/qq_27607965/article/details/79925288)
...
#### 4.2.6、 MyISAM和InnoDB在索引上的差别及其它区别
...
![mysql19](../images/mysql19.png)

　辅助索引：以另一字段索引到主键

![mysql19_1](../images/mysql19_1.png)
...
![mysql19_2](../images/mysql19_2.png)

辅助索引：以某字段索引到记录地址

![mysql19_3](../images/mysql19_3.png)
...
参考回答：
1）字符串string：整数值、embstr编码的简单动态字符串、简单动态字符串（SDS)
...
#### 5.1.10、首先要知道Hash索引和B+树索引的底层实现原理:
